...
const Server = function (db, name, version, noServerMethods) {
  ...
  [].some.call(db.objectStoreNames, storeName => {
    ...
    keys
      .filter(key => ![...serverEvents, "close", "addEventListener", "removeEventListener"].includes(key))
      .map(key => (this[storeName][key] = (...args) => this[key](storeName, ...args)));
  });
  return err;
};
        ...
        request.onblocked = e => {
          const resume = new Promise(function (res, rej) {
            ...
            request.onsuccess = ev => {
              open(ev, server, version, noServerMethods).then(res, rej);
            };
